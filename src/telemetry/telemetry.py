...
class TelemetryService:
    ...
    async def trace_agent_execution(
        agent_name: str, invoice_id: str, metadata: dict[str, Any] | None = None
        ...
        with tracer.start_as_current_span(f"AgentExecution:{agent_name}") as span:
            ...
            try:
                ...
                logger.info(
                    f"✅ [AGENT COMPLETE] {agent_name} in {duration:.2f}ms | Tools: {len(metrics['tool_calls'])}"
                )
    ...
    def log_tool_call(
        ...
        logger.info(
            f"🛠 [TOOL CALL] {agent_name} -> {tool_name} ({execution_time_ms:.2f}ms) | Args: {inputs}"
        )
