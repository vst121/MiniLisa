...
class SupplierRisk(BaseModel):
    ...
    supplier_id: Optional[str] = Field(default=None, description="Database ID of matching supplier")
    supplier_name: str = Field(description="Matched or identified supplier name")
    tax_id: Optional[str] = Field(default=None, description="Tax ID or VAT number")
    risk_score: float = Field(default=0.0, description="Risk score from 0.0 (safe) to 100.0 (high risk)")
    ...
    payment_history_status: str = Field(default="GOOD", description="Payment history flag e.g. GOOD, DELINQUENT, NEW")
    ...
    summary: str = Field(default="", description="Executive summary of supplier intelligence assessment")
